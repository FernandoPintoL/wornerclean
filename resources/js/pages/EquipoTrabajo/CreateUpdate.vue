...
import AssignEmpleadosToEquipoTrabajo from '@/Componentes/AssignEmpleadosToEquipoTrabajo.vue';

const negocio = new EquipoTrabajoNegocio();
const model_path = negocio.model;

// Reference to store the equipo trabajo ID for the employee assignment component
const equipoTrabajoId = ref<number | null>(null);
...
const props = defineProps({
    model: Object as () => EquipoTrabajo | null,
    isCreate: {
        type: Boolean,
        default: false,
    },
...
onMounted(() => {
    if (props.model) {
        ...
        // Set the equipo trabajo ID for the employee assignment component
        equipoTrabajoId.value = props.model.id;
    }
});
    ...
    try {
        ...
        if (response.isSuccess) {
            ...
            if (response.data && response.data.id) {
                equipoTrabajoId.value = response.data.id;
            }

            await AlertService.success('La operación se completó con éxito.').then(() => {
                // If we're in create mode, redirect to edit mode to allow employee assignment
                if (props.isCreate) {
                    window.location.href = route(model_path + '.edit', { [model_path]: response.data.id });
                } else {
...
const editar_model = async () => {
    try {
        ...
        if (response.isSuccess) {
            ...
            await AlertService.success('La operación se completó con éxito.').then(() => {
                // Stay on the same page to allow continued employee management
                // or redirect to index if preferred
                window.location.href = route(model_path + '.index');
            });
        } else {
            await AlertService.error(response.message);
        ...
        <div class="p-4 sm:p-6 lg:p-8 transition-all duration-300">
            <div class="w-full max-w-7xl mx-auto">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6 transition-all duration-300 animate-fadeIn">
                    ...
                    <!-- Employee assignment section (only visible in edit mode) -->
                    <div v-if="!props.isCreate && equipoTrabajoId" class="mt-8 animate-fadeIn" style="animation-delay: 0.6s;">
                        <div class="relative my-6">
                            <div class="absolute inset-0 flex items-center">
                                <div class="w-full border-t border-gray-200 dark:border-gray-700"></div>
                            </div>
                        ...
                        <AssignEmpleadosToEquipoTrabajo
