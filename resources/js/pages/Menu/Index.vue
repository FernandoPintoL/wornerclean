...
import { MenuLink, getDefaultAttributes, selectedAttributes } from '@/Data/Menu';
...
const datas = reactive({
    ...
    totalPages: 1,
    perPage: 5,
    menuMap: new Map<number, string>(), // Map to store menu id -> title for parent lookup
});
...
const fetchList = async (page = 1) => {
    ...
    if (response.isSuccess) {
        ...
        // Build menu map for parent lookup
        datas.menuMap.clear();
        response.data.data.forEach((menu: MenuLink) => {
            if (menu.id) {
                datas.menuMap.set(menu.id, menu.title);
            }
        });
    } else {
        datas.list = [];
...
// Get parent menu title from ID
const getParentMenuTitle = (parentId: number | null): string => {
    if (!parentId) return 'N/A';
    return datas.menuMap.get(parentId) || `ID: ${parentId}`;
};
            ...
            <div class="w-full max-w-7xl mx-auto">
                ...
                <div v-if="datas.list.length === 0 && !datas.isLoad" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 my-4 animate-fadeIn-delayed">
                    <div class="text-center py-8">
                        ...
                        <Alert v-if="query.length === 0" :message="'No se encontraron menús'" :path="model + '.create'" />
                        <Alert-Info v-else :message="'No se encontraron menús con: ' + query" />
                    </div>
                </div>
                        ...
                        <TableLayout :mostrarfoot="datas.list.length > 10">
                            <template #thead>
                                <tr class="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 dark:from-indigo-900/30 dark:to-purple-900/30">
                                    ...
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Título</th>
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Enlace</th>
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Icono</th>
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Principal</th>
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Es Submenú</th>
                                    <th scope="col" class="p-4 text-left text-xs font-medium text-gray-700 uppercase tracking-wider dark:text-gray-300">Menú Padre</th>
                            ...
                            <template #tbody>
                                ...
                                >
                                    ...
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Título">
                                        {{ item.title }}
                                    </td>
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Enlace">
                                        {{ item.href }}
                                    </td>
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Icono">
                                        {{ item.icon }}
                                    </td>
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Principal">
                                        {{ item.isMain ? 'Sí' : 'No' }}
                                    </td>
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Es Submenú">
                                        {{ item.isSubmenu ? 'Sí' : 'No' }}
                                    </td>
                                    <td class="p-4 text-sm md:text-base font-medium text-gray-900 dark:text-white" data-label="Menú Padre">
                                        {{ getParentMenuTitle(Number(item.parent_id) ?? 0) }}
                                    </td>
